...
def demo_clip(seconds: float = 8.0, sample_rate: int = 22050) -> AudioClip:
    ...
    phrases = np.clip(0.55 + 0.6 * np.sin(t * 2.3 + 0.4) * np.sin(t * 0.9 + 1.1) + 0.15 * np.sin(t * 9.7), 0, 1) ** 1.5
    ...
    breath = rng.normal(0, 0.06, n)
    mono = (voiced * 0.45 + breath) * phrases

    left = mono + 0.10 * np.sin(2 * np.pi * 3.0 * t) * mono
    right = np.roll(mono, 90) * 0.92 + 0.05 * rng.normal(0, 0.05, n)  # slight delay: a believable stereo image
    stereo = np.stack([left, right]).astype(np.float32)
    return AudioClip(stereo / max(np.abs(stereo).max(), 1e-6) * 0.85, sample_rate)


def demo_peaks(buckets: int, clip: AudioClip | None = None, start: float = 0.0, stop: float | None = None) -> Peaks:
